...
1. Wait for it to [initialize](https://docs.aws.amazon.com/ebs/latest/userguide/ebs-initialize-monitor.html)
  - TODO: See if there is a way to handle this automatically (i.e. with IaC)
1. Depending on the guest OS, the device name could be remapped
  - In the case of Debian, it remaps NVME devices to `xvd*` as per these docs
    -  There may be docs that specify this
    - NOTE: this is notated in AWS EBS Volume usage docs; however, the current documentation implicitly assumes that the reader will make the distinction of OS device mappings.
